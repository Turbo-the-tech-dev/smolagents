...
class TestPullMessagesFromStep:
    ...
    def test_final_answer_step_audio(self):
        ...
        # Use a real AgentAudio instance but mock its __init__ to avoid torch/soundfile dependency
        with patch("smolagents.agent_types.AgentAudio.__init__", return_value=None):
            with patch("smolagents.agent_types.AgentAudio.to_string", return_value="path/to/audio.wav"):
                audio = AgentAudio("mock_path")
                step = FinalAnswerStep(output=audio)
                messages = list(pull_messages_from_step(step))
                assert len(messages) == 1
