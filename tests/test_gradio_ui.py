...
class GradioUITester(unittest.TestCase):
    ...
    def test_upload_file_success(self, mock_info):
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt") as temp_file:
            ...
            mock_info.assert_called_with(f"File uploaded successfully: {os.path.basename(temp_file.name)}")
            self.assertEqual(len(uploads_log), 1)
            self.assertTrue(os.path.exists(os.path.join(self.temp_dir, os.path.basename(temp_file.name))))
            self.assertEqual(uploads_log[0], os.path.join(self.temp_dir, os.path.basename(temp_file.name)))

    @patch("gradio.Warning")
    def test_upload_file_none(self, mock_warning):
        """Test scenario when no file is selected"""
        uploads_log = self.ui.upload_file(None, [])

        mock_warning.assert_called_with("No file uploaded")
        self.assertEqual(len(uploads_log), 0)
...
class TestPullMessagesFromStep:
    ...
    def test_final_answer_step_audio(self):
        ...
        mock_audio = Mock(spec=AgentAudio)
        mock_audio.to_string.return_value = "path/to/audio.wav"
        step = FinalAnswerStep(output=mock_audio)

        with patch("smolagents.gradio_ui.isinstance") as mock_isinstance:

            def side_effect(obj, cls):
                if cls == AgentAudio and obj == mock_audio:
                    return True
