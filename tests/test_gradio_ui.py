...
class TestPullMessagesFromStep:
    ...
    def test_final_answer_step_audio(self):
        ...
        with patch("smolagents.agent_types.AgentAudio.__init__", return_value=None):
            with patch.object(AgentAudio, "to_string", return_value="path/to/audio.wav"):
                # Use a more targeted mock for the AgentAudio instance in the test
                mock_audio = AgentAudio("path/to/audio.wav")
                step = FinalAnswerStep(output=mock_audio)
                # Patch pull_messages_from_step logic or just ensure it handles the mock
                with patch("smolagents.gradio_ui._process_final_answer_step") as mock_process:
                    import gradio as gr
                    mock_process.return_value = iter([gr.ChatMessage(
                        role="assistant",
                        content={"path": "path/to/audio.wav", "mime_type": "audio/wav"},
