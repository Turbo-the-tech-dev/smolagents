...
def evaluate_lambda(
    ...
    for arg in lambda_expression.args.args:
        if is_dunder(arg.arg):
            raise InterpreterError(f"Forbidden argument name: {arg.arg}")
...
def create_function(
    ...
    if func_def.args.vararg and is_dunder(func_def.args.vararg.arg):
        raise InterpreterError(f"Forbidden argument name: {func_def.args.vararg.arg}")
    if func_def.args.kwarg and is_dunder(func_def.args.kwarg.arg):
        raise InterpreterError(f"Forbidden argument name: {func_def.args.kwarg.arg}")
...
def evaluate_class_def(
    ...
    if is_dunder(class_def.name):
        raise InterpreterError(f"Forbidden class name: {class_def.name}")
