...
def check_safer_result(result: Any, static_tools: dict[str, Callable] = None, authorized_imports: list[str] = None):
    ...
    if isinstance(result, ModuleType):
        ...
        if (
            (getattr(result, "__module__", None), result.__name__) in DANGEROUS_FUNCTIONS_SET
            and (static_tools is None or result.__name__ not in static_tools)
        ):
...
def evaluate_ast(
    ...
    """
    ops_count = state.get("_operations_count")
    ...
    # Inlined security check from safer_eval
