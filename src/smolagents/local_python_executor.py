...
DANGEROUS_FUNCTIONS_SET = set(tuple(f.rsplit(".", 1)) for f in DANGEROUS_FUNCTIONS)
...
def check_safer_result(result: Any, static_tools: dict[str, Callable] = None, authorized_imports: list[str] = None):
    ...
    if isinstance(result, ModuleType):
        ...
        if result.__name__ not in DANGEROUS_FUNCTIONS_NAMES:
            return
        if (getattr(result, "__module__", None), result.__name__) in DANGEROUS_FUNCTIONS_SET:
            if static_tools is None or result.__name__ not in static_tools:
...
def evaluate_ast(
    ...
    """
    # Use a direct local variable for the operations count to avoid dictionary lookups in the hot path
    if "_operations_count" in state:
        ops = state["_operations_count"]
    else:
        state["_operations_count"] = {"counter": 0}
        ops = state["_operations_count"]

    if ops["counter"] >= MAX_OPERATIONS:
        raise InterpreterError(
    ...
    if handler:
        ...
        if expression_type in {ast.Call, ast.Name, ast.Attribute, ast.Subscript}:
