...
def check_arguments(args: ast.arguments):
    for arg in args.args:
        check_dunder_name(arg.arg)
    for arg in args.kwonlyargs:
        check_dunder_name(arg.arg)
    if args.vararg:
        check_dunder_name(args.vararg.arg)
    if args.kwarg:
        check_dunder_name(args.kwarg.arg)
...
def evaluate_attribute(
    ...
    check_dunder_name(expression.attr)
...
def evaluate_lambda(
    ...
    check_arguments(lambda_expression.args)
...
def evaluate_with(
    ...
    for item in with_node.items:
        context_mgr = evaluate_ast(item.context_expr, state, static_tools, custom_tools, authorized_imports)
        contexts.append(context_mgr)
        enter_result = context_mgr.__enter__()
            ...
            if expression.names[0].name == "*":  # Handle "from module import *"
                if hasattr(module, "__all__"):  # If module has __all__, import only those names
                    for name in module.__all__:
                        if not is_dunder(name) or name in ALLOWED_DUNDER_METHODS:
