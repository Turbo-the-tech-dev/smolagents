...
def create_function(
    ...
    for arg in func_def.args.args + func_def.args.kwonlyargs + [func_def.args.vararg, func_def.args.kwarg]:
        if arg and is_dunder(arg.arg):
            raise InterpreterError(f"Forbidden dunder name in function arguments: {arg.arg}")
...
def evaluate_function_def(
    ...
    if is_dunder(func_def.name) and func_def.name not in ALLOWED_DUNDER_METHODS:
        raise InterpreterError(f"Forbidden dunder name in function definition: {func_def.name}")
...
def evaluate_class_def(
    ...
    if is_dunder(class_name):
        raise InterpreterError(f"Forbidden dunder name in class definition: {class_name}")
    ...
    for stmt in class_def.body:
        if isinstance(stmt, ast.FunctionDef):
            if is_dunder(stmt.name) and stmt.name not in ALLOWED_DUNDER_METHODS:
                raise InterpreterError(
                    f"Forbidden dunder name in class body: {stmt.name}. Only {ALLOWED_DUNDER_METHODS} are allowed."
                )
            ...
            if isinstance(target, ast.Name):
                if is_dunder(target.id) and target.id != "__annotations__":
                    raise InterpreterError(f"Forbidden dunder name in class body: {target.id}")
...
def set_value(
    ...
    if isinstance(target, ast.Name):
        if is_dunder(target.id):
            raise InterpreterError(f"Forbidden access to dunder name: {target.id}")
            ...
            if handler.type is None or isinstance(
                ...
                if handler.name:
                    if is_dunder(handler.name):
                        raise InterpreterError(f"Forbidden dunder name in exception handler: {handler.name}")
...
def evaluate_with(
    ...
    for item in with_node.items:
        ...
        if item.optional_vars:
            if isinstance(item.optional_vars, ast.Name) and is_dunder(item.optional_vars.id):
                raise InterpreterError(f"Forbidden dunder name in with statement: {item.optional_vars.id}")
...
def evaluate_import(expression, state, static_tools, custom_tools, authorized_imports):
    if isinstance(expression, ast.Import):
        for alias in expression.names:
            ...
            if alias.asname and is_dunder(alias.asname):
                raise InterpreterError(f"Forbidden alias to dunder name: {alias.asname}")
