...
def evaluate_with(
    ...
    for item in with_node.items:
        ...
        if item.optional_vars:
            set_value(item.optional_vars, context_expr.__enter__(), state, static_tools, custom_tools, authorized_imports)
    ...
    if isinstance(expression, ast.Import):
        for alias in expression.names:
            if is_dunder(alias.name):
                raise InterpreterError(f"Forbidden access to dunder attribute: {alias.name}")
            if alias.asname and is_dunder(alias.asname):
                raise InterpreterError(f"Forbidden access to dunder attribute: {alias.asname}")
