...
def evaluate_lambda(
    ...
    for arg in lambda_expression.args.args:
        if is_dunder(arg.arg):
            raise InterpreterError(f"Forbidden dunder name as argument: {arg.arg}")
        args.append(arg.arg)
...
def create_function(
    ...
    if func_def.args.vararg:
        if is_dunder(func_def.args.vararg.arg):
            raise InterpreterError(f"Forbidden dunder name as argument: {func_def.args.vararg.arg}")

    if func_def.args.kwarg:
        if is_dunder(func_def.args.kwarg.arg):
            raise InterpreterError(f"Forbidden dunder name as argument: {func_def.args.kwarg.arg}")
...
def evaluate_function_def(
    ...
    if is_dunder(func_def.name) and func_def.name not in ALLOWED_DUNDER_METHODS:
        raise InterpreterError(f"Forbidden access to dunder name: {func_def.name}")
...
def evaluate_class_def(
    ...
    if is_dunder(class_name):
        raise InterpreterError(f"Forbidden access to dunder name: {class_name}")
            ...
            if handler.type is None or isinstance(
                ...
                if handler.name:
                    if is_dunder(handler.name):
                        raise InterpreterError(f"Forbidden access to dunder name: {handler.name}")
...
def evaluate_with(
    ...
    for item in with_node.items:
        ...
        if item.optional_vars:
            context_manager_value = context_expr.__enter__()
            set_value(item.optional_vars, context_manager_value, state, static_tools, custom_tools, authorized_imports)
            contexts.append(context_manager_value)
        else:
    ...
    if isinstance(expression, ast.Import):
        for alias in expression.names:
            if check_import_authorized(alias.name, authorized_imports):
                ...
                if is_dunder(import_name):
                    raise InterpreterError(f"Forbidden import of dunder name: {import_name}")
                raw_module = import_module(alias.name)
                state[import_name] = get_safe_module(raw_module, authorized_imports)
            else:
                raise InterpreterError(
            ...
            if expression.names[0].name == "*":  # Handle "from module import *"
                ...
                for alias in expression.names:
                    ...
                    if hasattr(module, alias.name):
                        state[import_name] = getattr(module, alias.name)
                    else:
